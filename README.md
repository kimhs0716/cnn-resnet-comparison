...
**Augmentation 없이는 PlainCNN이 우세하나, augmentation 적용 시 ResNet이 역전되며 깊이가 깊어질수록 격차가 커진다.**

| 조건 | CIFAR-10 | CIFAR-100 |
|------|----------|-----------|
| No aug | PlainCNN 우세 | PlainCNN 우세 |
| Aug (shallow) | ResNet 우세 | ResNet 우세 |
...
- **깊이가 증가할수록 ResNet의 우위가 커짐** — residual connection의 degradation 방지 효과
- **복잡한 태스크(CIFAR-100)에서도 동일한 방향성 확인**

### 한계

로컬 환경(Intel XPU)의 oneDNN 비결정성으로 인해 실행마다 수치 차이가 존재한다. 개별 수치보다는 방향성을 중심으로 해석해야 한다.
